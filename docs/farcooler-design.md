# Design: Far Cooler, a Mobile Command Center for Parallel Coding Agents
...
Far Cooler should let an engineer onboard macOS, Linux, and WSL2 hosts; create repositories and task workspaces on those hosts; launch Claude Code, Codex, or Cursor CLI in parallel; attach directly to their terminals; and move between an iPhone and a Mac without interrupting the underlying work. The host daemon, not a thin client, owns authoritative workspace, process, and session state so an iPhone can disappear offline and another device can pick up the same work.
...
- Cursor, Anthropic, OpenAI, Superset, Conductor, and numerous open-source projects validate the category. This is market evidence, not proof that Far Cooler itself has demand.
...
- **User-owned compute:** The daemon runs on the user's macOS, Linux, and WSL2 hosts. Far Cooler does not require managed development VMs.
- **SSH control plane:** SSH stdio is the only control-plane transport. The daemon binds no port on any interface; it accepts connections on its local Unix socket and connections that sshd launches on its behalf. Users reuse their existing host keys, agents, bastions, `ProxyJump`, and access policy. Tailscale keeps its full value as the network layer, because an SSH route to a MagicDNS name still gets NAT traversal and stable addressing, but tailnet membership is a path and never an authorization. There is no second data-plane transport: the terminal channel's own byte-exact resume and replay buffer cover reconnect.
- **A second network path, and no second protocol:** a runner may also hold an outbound tunnel (`crates/tailcat`) that an admitted device dials to reach that runner's own sshd on loopback, so a box behind NAT needs no address and no inbound port. What arrives is a connection into the same sshd, which launches the same stdio process and applies the same `authorized_keys`; the tunnel is addressing, never authorization, exactly as tailnet membership is. Admission to it is one node key per enrolled device, carried on that device's `authorized_keys` line, and a runner whose devices carry no node key serves no tunnel at all — which is every runner today: the only way a line gains a node key is a device calling `client.set_node_key`, no shipped client calls it, and where a device's own node key would come from is still an open decision.
- **Reuse before reinvention:** Prefer established primitives such as OpenSSH, tmux, git worktrees, launchd/systemd, SQLite, and Tailscale when they satisfy the security and experience contract. Far Cooler adds adapters and durable product semantics instead of forking or disguising those tools.
...
- **Security:** Tailscale membership alone is not sufficient authorization. Every device authenticates with its own SSH key, is enrolled with an explicit scope, and can be revoked individually. Far Cooler does not operate a certificate authority or issue client certificates.
...
5. SSH provides transport, mutual authentication, and device credentials; Tailscale provides the network path where the user wants one. Far Cooler owns enrollment scope, worktree lifecycle, process lifecycle, authoritative workspace state, notification policy, and the client experience. It does not build its own authentication system.
...
SSH, terminal multiplexers, remote IDEs, git worktrees, and private mesh networks already solve pieces of the system. Far Cooler should compose these patterns rather than invent new networking, terminal persistence, or source-control primitives. In particular, tmux's detach/reattach and control-mode concepts are the accepted backend, and Far Cooler's own terminal channel provides the durable reconnect behavior through byte-exact resume and visible gap markers.
...
### Layer 3: Far Cooler's thesis

Current products tend to be strongest at either vendor-specific mobile control or desktop parallel orchestration. Far Cooler's thesis is that engineers will value a single terminal-first mobile command center for parallel workspaces across their own hosts enough to adopt a new daemon and client.
...
### Approach B: Complete Far Cooler Control Plane
...
- A dedicated tmux server namespace such as `tmux -L farcooler-<install-id> -f /dev/null`, containing one host-wide managed session. Far Cooler never mixes managed windows into the user's default tmux server or depends on the user's tmux configuration.
- The daemon uses tmux control mode and stable tmux IDs while the Mac app renders its own hierarchy and controls. Its terminal surface uses the shared Rust terminal core through a Far Cooler-owned C ABI.
- `farcooler attach WORKSPACE_ID` remains mandatory and attaches an ordinary shell client to the managed session. The equivalent raw tmux command is shown for transparency and recovery.
- State ownership splits by durability. SQLite stores only what must outlive tmux: repository roots, repositories, workspaces with their worktree and branch, terminal durable identity and intent, agent session IDs, operations, and audit records. tmux is the sole authority for whether a managed process is alive right now. The daemon never stores a terminal runtime state field, so it cannot serve a stale `running`; runtime state is derived per request from intent plus a live exactly-tagged pane, and an untagged or unmatched window is never treated as a Far Cooler object automatically.
...
Far Cooler CLI ── Unix socket ─┼── transport adapter ── one daemon protocol/service
...
- Unsafe Rust is forbidden in Far Cooler-owned crates during the first slice. Any dependency requiring unsafe code is reviewed and pinned through the dependency policy.

The first slice may ship separate `farcoolerd` and `farcooler` executables from the same Cargo workspace. “Single statically distributable binary” means self-contained installation without a language runtime; it does not require collapsing daemon and CLI process roles or fully static macOS linking.
...
FileVault is the hard limit. Before the disk is unlocked no launchd job of any kind exists, so unattended mode closes the logout gap and cannot close the encrypted-reboot gap. Far Cooler does not suggest disabling FileVault or enabling auto-login to work around this, because both weaken at-rest protection on a machine holding source code and agent credentials.

The second limit is credentials. A daemon started before any login has no unlocked login keychain, so git operations and coding-agent CLIs that store credentials there fail until the user logs in once after boot. Far Cooler surfaces this as a specific, named condition rather than an opaque authentication failure, and the host reports it in its self-health reasons.
...
- Moving or replacing the app bundle, disabling its background item, version incompatibility, and deleting the app each have explicit detection and recovery tests in both registration modes. Uninstalling while unattended mode is active unregisters the system daemon, which prompts for administrator authorization once; an uninstall that cannot obtain it reports the exact leftover registration rather than leaving a silent orphan. Unregistering either registration stops only `farcoolerd`; removing user data, managed worktrees, or live tmux sessions requires separate exact confirmation and is never part of ordinary app deletion.
- Runtime data lives under `~/Library/Application Support/Far Cooler/` with user-only permissions. The Unix socket and identity/database files are created in user-only subdirectories and never inside the replaceable app bundle.
...
**The emulator behind the ABI is `alacritty_terminal`, not `libghostty-vt`.** The architecture above is unchanged — a single shared core, a Far Cooler-owned boundary, thin native renderers — and this is precisely the substitution that owning the boundary was meant to allow. The reasons for the change:
...
Android compilation and rendered-fixture tests are architecture validation for this Mac-first slice, not a user-ready Android client. If the Android build or input/render contract fails, terminal architecture is reopened before the Mac vertical slice expands. Because the ABI is Far Cooler's own rather than an upstream one, an emulator upgrade is contained to `crates/vt` and cannot reach a renderer.
...
MVP status is based only on facts the daemon can observe. Far Cooler does **not** claim that a generic terminal is waiting for approval or user input.
...
Far Cooler has no `Server` resource, no port metadata, and no browser handoff. A development server is a long-lived command in an ordinary Far Cooler terminal, with the same output, scrollback, restart, and `lost` semantics as any other terminal, and nothing tracks it as a distinct object.

Reaching that server from a phone needs no product feature. Tailscale already supplies the network path, so `http://host-magicdns-name:5173` works in Safari with no Far Cooler involvement, and users who prefer it can forward a port over their existing SSH access or expose it with a tunnel of their own choosing. A port field, a liveness probe, and a second lifecycle would have bought a URL the user can already type.
...
For MVP, the CLI integrations are launch presets inside real terminals. Far Cooler does not parse or emulate each vendor's entire interaction model.
...
Far Cooler's conceptual model is “tmux for the GUI and coding-agent era”: the daemon owns durable workspaces and sessions, while each thin client owns only device-specific presentation such as visible split sizes, selected tab, font size, and keyboard toolbar state. Reusing tmux underneath does not mean displaying tmux's interface; React Native and Swift clients render the complete Far Cooler UI from daemon resources and terminal frames.

The accepted MVP backend launches one private tmux server under a daemon-specific socket and minimal configuration rather than attaching to or mutating the user's ordinary tmux server. It creates one host-wide managed session, named `farcooler` for display but addressed internally by its stable tmux session ID. A workspace is a daemon/SQLite grouping of tagged tmux windows, not a tmux session. Each terminal tab maps to one window containing one pane for MVP; later split panes may add panes within that terminal window.

```text
private tmux server
└── host session: farcooler
...
| Shell escape hatch | Required | `farcooler attach` and raw `tmux attach` both reach the live session from an ordinary SSH client |
...
The host session is tagged with `@farcooler_daemon_id` and `@farcooler_schema_version`. Every managed window and pane is tagged with `@farcooler_daemon_id`, `@farcooler_workspace_id`, `@farcooler_terminal_id`, and `@farcooler_schema_version`. Names, indexes, and PID values are display or diagnostic data only and never establish identity.
...
Correctness therefore depends on the parser handling every relevant notification, which makes a missed notification type a silent staleness bug. A low-frequency full reconcile runs as a backstop. Any divergence it finds is logged as a defect with the notification that should have arrived, not silently corrected, because silent correction would hide exactly the bug the backstop exists to catch.
...
The tmux backend has one unusually valuable acceptance requirement: shell portability. `farcooler attach WORKSPACE_ID` runs locally on the host, attaches to the host session, and selects the workspace's last active window. The raw recovery command `tmux -L farcooler-<install-id> attach -t farcooler` exposes the flat list of every managed terminal on the host. Therefore a user on an unsupported **client** operating system needs only an ordinary SSH client:

```text
ssh user@host farcooler attach WORKSPACE_ID
```

**`farcooler attach` acquires no writer lease and is outside lease enforcement entirely.** It is a tmux client, and a tmux client can switch to any window in the session and deliver keystrokes there, so a lease scoped to anything narrower than the whole session would be false. Rather than claim an enforcement it cannot deliver, Far Cooler documents the escape hatch as a bypass with the same standing as raw `tmux attach`.
...
- `automatic_after_infrastructure_loss` is the default. After host reboot, tmux loss, or another verified infrastructure failure, Far Cooler starts the documented exact-ID resume command and stops at the restored interactive prompt. It never fabricates or resubmits the last user input.
...
- One protocol client holds the writer lease for a terminal; all other attached protocol clients are read-only. The lease binds clients speaking the Far Cooler protocol. It does not bind `farcooler attach` or raw tmux, which are same-user shells outside enforcement.
...
The terminal renderer is the highest-risk technical spike. `crates/vt` behind the Far Cooler C ABI is the accepted shared terminal core; platform-native render and input adapters must still be validated on iOS and Android for latency, keyboard behavior, selection, accessibility, and large scrollback. React Native remains the mobile application framework while the terminal surface is a native module.
...
Every enrolled host is connected at once, not one at a time. Workspaces from all of them appear in a single sidebar, each naming the host it runs on, and every action routes back to that host. There is no "current machine" to switch between first: an agent blocked on a server in another room is exactly as visible, and exactly as actionable, as one running on the desk in front of you. A host that stops answering keeps its rows in place, dimmed, with its own connection state named on its project headers — reads keep showing the last good fetch, and an action against that host is refused immediately with its own error rather than hanging or being queued for later. Nothing is ever queued: an action replayed minutes afterward against state that has since changed is a surprise nobody could have attributed to anything they did.
...
`host_admin` is an application safety rail, not containment. A `control` client writes bytes into a real shell running as the daemon's own operating-system user, so it can read `~/Library/Application Support/FarCooler/`, the identity key, and the SQLite database, invoke the `farcooler` CLI over the local socket, or run any other program as that user. Everything `host_admin` protects is reachable by going around the API instead of through it. Far Cooler therefore does not claim that a hostile `control` client is contained beneath `host_admin`, and no part of the product may be documented as if it were.
...
Because `control` carries full host-user authority, enrollment enforces the distinction in the product surface rather than only in documentation. Enrolling a device at `control` or `host_admin` displays the plain-language warning that the device gains the ability to run any command as the host user. `read` is offered as the safe choice for a device the user does not fully trust, and it is enforced by sshd rather than by Far Cooler alone: a read-enrolled key carries `restrict` plus a forced command, so it cannot open an interactive SSH shell even if the daemon is compromised.

Revocation is the containment response to a compromised `control` device, so revocation latency is a success criterion and revocation is reachable both through the protocol and from a host shell. Revocation ends Far Cooler-mediated access and removes the enrolled key. It does not undo what that device already did through a terminal: a `control` client with shell authority can install its own persistence that Far Cooler never observes. The documented recovery is revoke first, then audit the host.

This posture matches Far Cooler's initial threat model: a single developer's personal hosts, where every enrolled device is one the user already trusts with their laptop. A real containment boundary would require a separate operating-system principal for daemon administration with sandboxed terminal workers and narrow IPC between them. That is deliberately deferred; see "Deferred."
...
- **SSH stdio:** the only control-plane adapter. The client verifies the SSH host key, authenticates with its enrolled device key, and sshd launches the forced `farcooler transport stdio` command. SSH configuration may name a bastion or `ProxyJump`; Far Cooler never copies private keys to the daemon. The client identity and scope that bind audit records, writer leases, and idempotency records come from the enrolled key that sshd authenticated.
...
- **No second data-plane transport.** Far Cooler ships one transport. The roaming and resume behavior a second transport would provide is already in the terminal channel itself: byte-exact resume within an epoch, an 8 MiB replay buffer, and honest gap markers when it cannot cover a gap.
...
- **Replay memory is per terminal and grows linearly, which is a deliberate choice with a stated cost.** Twenty terminals, the shape of five workspaces with four tabs each that success criterion 3 requires, is roughly 160 MiB of daemon memory held solely for reconnect. That is the dominant term in the daemon's footprint and most of it belongs to terminals nobody is currently watching. The fixed per-terminal rule is kept because it is predictable, because reconnect depth never depends on what other terminals are doing, and because it needs no eviction or fairness policy that a noisy neighbour could exploit. The daemon reports total replay usage in its self-health so the number is observable rather than a surprise, and a global budget with fair allocation is the known answer if real usage makes the footprint a problem.
...
- The control-mode client never calls `refresh-client -C`, so its own pseudo-terminal dimensions cannot affect managed windows. The managed session sets `window-size latest`, so tmux sizes a window to its most recently active client, which is the same rule the protocol uses. `farcooler attach` forwards the attached TTY's initial size and `SIGWINCH` changes on that basis, and raw tmux access participates in the same tmux-level rule while remaining outside writer-lease enforcement.

`farcooler protocol inspect` can decode a captured length-delimited stream or individual protobuf envelope into redacted protobuf JSON for debugging. It hides terminal input/output and path-bearing fields unless the user passes an explicit local reveal flag.
...
SSH is Far Cooler's only control-plane transport and its only authentication mechanism. Far Cooler does not run a certificate authority, issue client certificates, or pin its own identity key. OpenSSH already authenticates both directions, and D17 established that the asset being protected is a shell that SSH gates anyway. Delegating removes an entire security subsystem Far Cooler would otherwise own for the life of the product. The clause this sentence used to end with — "or operate a QR pairing flow" — was true of the design that deleted the certificate authority and is not true of what shipped: device onboarding IS a QR ceremony, on all three clients. What it carries is an SSH public key onto a fenced `authorized_keys` line, so the subsystem stayed deleted; the flow came back without it.
...
4. **Enrollment.** Far Cooler-managed entries live in a fenced block inside `~/.ssh/authorized_keys`. Every enrolled entry carries `restrict` plus `command="~/.local/bin/DAEMON --stdio --client CLIENT_ID --scope SCOPE"`, where `DAEMON` is the channel's own binary name, so a Far Cooler device key reaches the daemon protocol and cannot open an interactive SSH shell, request port forwarding, or allocate a PTY. The path is not decoration: a forced command runs through the account's login shell, so a bare name is resolved by whatever that shell leaves on `PATH`, and the channel name keeps a canary daemon from enrolling a device against the stable install. The daemon revalidates the client and scope itself and never trusts a client-supplied identity.
...
- The daemon binds no port on any interface. Its only entry points are a mode-`0600` Unix socket and processes that sshd starts after authenticating a device key. There is no TLS stack, no WebSocket server, and no port to expose, scan, or misconfigure.
- A runner that serves a tunnel holds one outbound connection through which admitted devices reach the sshd on its host. Two consequences are accepted rather than mitigated, and are stated in `docs/runners.md` where somebody deciding whether to enroll a device will read them. First, the tunnel forwards to the host's sshd rather than to one runner's account, so runners sharing a box now share a tunnel entrance — every account's `authorized_keys` still gates that account, so nothing is granted, but the entrance is one thing they share. Second, a runner cannot verify that a device holds the private half of the node key it registers, so an enrolled device can extend tunnel reachability to a machine that is not enrolled; what that machine reaches is an sshd that refuses it, and revocation removes the device's line and its node key together. A proof of possession is not the answer — the risk is delegation rather than impersonation, and a node key is a clamped X25519 key that nothing can sign with.
- Far Cooler-enrolled device keys are restricted to the daemon subsystem and cannot open an interactive SSH shell. This genuinely constrains a `read` device. It does not reduce `control`, which reaches a shell by writing into a managed terminal, exactly as D17 describes.
- `~/.ssh/authorized_keys` is a user-critical file. Far Cooler edits only inside its own fence markers, atomically, with a checksummed backup, and refuses to edit a file whose fence it cannot verify. Losing SSH access to your own machine because of a management action is treated as a release-blocking failure and has its own tests.
- Repository roots are explicitly allowlisted. Far Cooler rejects arbitrary path traversal outside them. The allowlist constrains Far Cooler's own git and worktree operations; it does not constrain what a shell running inside a terminal can reach, because that shell has the host user's full filesystem access.
...
- A compromised enrolled control client is contained only by its daemon scopes and host-user permissions. Revocation is the immediate response; Far Cooler cannot make an arbitrary shell intrinsically safe.
...
| Enrollment fails or `authorized_keys` fence is damaged | Exact file state and the entry Far Cooler intended to write | Apply the printed entry manually, or repair the fence and retry |
...
- Uninstall stops the service, removes the binary, and removes Far Cooler's fenced block from `~/.ssh/authorized_keys` while leaving every other entry untouched. Repositories, worktrees, branches, and `~/.farcooler` metadata are preserved by default.
- `farcooler uninstall --purge-metadata` removes Far Cooler metadata and enrollment records only after an exact typed confirmation; it still does not delete repositories, branches, or worktrees.
...
- `ssh user@host farcooler attach WORKSPACE_ID` terminal escape hatch on unsupported client operating systems.
...
- An Far Cooler-owned certificate authority and a Tailscale-direct mutual-TLS listener. Delegating authentication to SSH removes the entire subsystem; a keyless direct route can return later if users ask for one, at which point it must re-justify the CA, rotation, expiry, and recovery work it brings with it.
...
11. **Accepted before Milestone 1:** Delegate all authentication to SSH. Delete the Far Cooler certificate authority, client certificates, identity pinning, and QR pairing flow, and drop the Tailscale-direct mutual-TLS listener; the daemon opens no network listener. Device identity is the enrolled SSH key fingerprint, scope is carried by a `restrict` plus forced-command entry in a fenced `authorized_keys` block, and revocation removes the entry and closes live connections. Tailscale remains a network path. Onboarding requires no presence at the host and installs the daemon remotely on Linux and WSL2. **Amended, 2026-09-04:** the decision stands and the delegation to SSH is unchanged, but "no network listener" is no longer the way to say it. A runner may hold an outbound tunnel that admitted devices dial to reach its host's sshd; nothing binds a port, and nothing skips sshd, so this is a second network path in the sense Tailscale already was. The rule is now phrased as "accepts no unauthenticated connection, and binds no port on any interface" — see `crates/transport/src/lib.rs`.
...
14. Revoking a client removes its enrolled key, closes its live connections, and blocks its next request within one second, without affecting other enrolled clients or any entry in `~/.ssh/authorized_keys` that Far Cooler did not write.
...
19. From the phone alone, a second Linux host with no Far Cooler installation is onboarded end to end: host key verified, daemon installed with a verified checksum, `systemd --user` unit enabled with lingering, device key enrolled, and a workspace created. A parallel fixture proves the daemon never modifies, reorders, or removes an `authorized_keys` entry outside its fence, and that a damaged fence produces refusal plus manual instructions rather than a rewrite.
20. **Deferred, gated on paid Apple Developer Program enrollment:** the same design partner repeats criterion 10 from an iPhone, on a device Far Cooler did not have to be rebuilt onto that week. Until this passes, the central product claim, that an engineer will supervise meaningful parallel coding work from a phone, is unvalidated, and no material may claim otherwise.
...
[+] crates/transport                                  ├── [GAP] Other clients unaffected
  ├── [GAP] same suite: in-memory | unix | ssh stdio  ├── [GAP] Non-Far Cooler authorized_keys untouched
...
**Mitigation:** Per-device SSH key enrollment, host-key verification with changed-key blocking, restricted forced-command entries, per-device revocation that closes live connections, secure Keychain key storage, scoped permissions, exact confirmation for destructive workspace actions, and no bound port on any interface — a runner's tunnel is an outbound connection into its own host's sshd, so every one of these mitigations applies to a device that arrives through it unchanged. The enrolled key that sshd authenticated supplies the client identity that binds writer leases and audit records. Far Cooler does not attempt to make a `control` device safe; it makes revocation fast and tells the user to audit the host afterward.
...
| Excluded | Why |
|---|---|
| Far Cooler certificate authority, client certificates, QR pairing, Tailscale-direct mTLS listener | SSH already authenticates both directions; the CA duplicated it and would be owned forever |
...
Record every tool, command, credential prompt, manual mapping, context switch, and return to the laptop. End by asking them to rank the three moments they most want Far Cooler to remove. This observation, not further architecture work, is the next step.
...
11. **Resolved in engineering review by removal:** declared servers are no longer a product concept. There is no `Server` resource, no port metadata, no liveness probe, and no browser handoff. A development server is a long-lived command in an ordinary terminal, and Tailscale already provides the network path that makes `http://host:PORT` work without any Far Cooler feature.

### Feasibility and security

12. **Resolved in engineering review:** the concern is accurate and is now stated in the design rather than papered over. `control` is full host authority as the daemon's OS user, `read` is the only genuine non-executing boundary, and `host_admin` is documented as an application safety rail that stops accidental destructive actions through Far Cooler's APIs, not as containment against a hostile `control` client. Enrollment warns in plain language when a device is granted `control` or `host_admin`; revocation is the containment response, and the documented recovery is revoke first, then audit the host, because revocation cannot undo what a shell already did. Moving daemon administration to a separate OS principal with sandboxed terminal workers is deferred with an explicit rationale.
