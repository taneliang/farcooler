...
SSH is Far Cooler's only control-plane transport and its only authentication mechanism. Far Cooler does not run a certificate authority, issue client certificates, pin its own identity key, or operate a QR pairing flow. OpenSSH already authenticates both directions, and D17 established that the asset being protected is a shell that SSH gates anyway. Delegating removes an entire security subsystem Far Cooler would otherwise own for the life of the product.
