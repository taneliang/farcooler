...
4. **Enrollment.** Far Cooler-managed entries live in a fenced block inside `~/.ssh/authorized_keys`. Every enrolled entry carries `restrict` plus `command="farcooler transport stdio --client CLIENT_ID --scope SCOPE"`, so a Far Cooler device key reaches the daemon protocol and cannot open an interactive SSH shell, request port forwarding, or allocate a PTY. The daemon revalidates the client and scope itself and never trusts a client-supplied identity.
