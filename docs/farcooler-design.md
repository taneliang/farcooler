...
- The daemon uses tmux control mode and stable tmux IDs while the Mac app renders its own hierarchy and controls. Its terminal surface uses the shared Rust terminal core through an Far Cooler-owned C ABI.
...
**The emulator behind the ABI is `alacritty_terminal`, not `libghostty-vt`.** The architecture above is unchanged — a single shared core, an Far Cooler-owned boundary, thin native renderers — and this is precisely the substitution that owning the boundary was meant to allow. The reasons for the change:
...
4. **Enrollment.** Far Cooler-managed entries live in a fenced block inside `~/.ssh/authorized_keys`. Every enrolled entry carries `restrict` plus `command="farcooler transport stdio --client CLIENT_ID --scope SCOPE"`, so an Far Cooler device key reaches the daemon protocol and cannot open an interactive SSH shell, request port forwarding, or allocate a PTY. The daemon revalidates the client and scope itself and never trusts a client-supplied identity.
