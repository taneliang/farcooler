...
**Mitigation:** Per-device SSH key enrollment, host-key verification with changed-key blocking, restricted forced-command entries, per-device revocation that closes live connections, secure Keychain key storage, scoped permissions, exact confirmation for destructive workspace actions, and no network listener at all. The enrolled key that sshd authenticated supplies the client identity that binds writer leases and audit records. Far Cooler does not attempt to make a `control` device safe; it makes revocation fast and tells the user to audit the host afterward.
