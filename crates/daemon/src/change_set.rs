...
/// The files Far Cooler wrote into the worktree itself don't appear here,
/// for `git::is_dirty`'s reason: each is named in the repository's
/// `info/exclude` when it is created, so a worktree this runner has just made
/// opens with nothing in its diff view that nobody put there, and the digest
/// `untracked_lines` and `worktree_digest` compute from this doesn't move when
/// the installer runs either.
pub async fn working_tree(repo: &Path) -> Result<WorkingTree> {
    let args = ["status", "--porcelain=v2", "--untracked-files=all", "-z"];
