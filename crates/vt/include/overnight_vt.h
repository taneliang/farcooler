/*
 * Overnight terminal core — the C ABI.
 *
 ...
 * Overnight's own interface, not a passthrough of whatever emulator sits
 ...
 *
 * Lifetime: pointers returned by overnight_vt_snapshot and overnight_vt_title
...
#ifndef OVERNIGHT_VT_H
#define OVERNIGHT_VT_H
...
#define OVERNIGHT_VT_FLAG_BOLD      (1u << 0)
#define OVERNIGHT_VT_FLAG_ITALIC    (1u << 1)
#define OVERNIGHT_VT_FLAG_UNDERLINE (1u << 2)
#define OVERNIGHT_VT_FLAG_INVERSE   (1u << 3)
/* A double-width character. The next column is its spacer; skip it. */
#define OVERNIGHT_VT_FLAG_WIDE      (1u << 4)
...
void *overnight_vt_new(uint16_t columns, uint16_t rows);

/* Destroy a terminal. Safe with NULL; never call twice on the same handle. */
void overnight_vt_free(void *handle);
 ...
 * socket read.
 */
void overnight_vt_feed(void *handle, const uint8_t *bytes, size_t len);

/* Resize the grid. The core reflows; the renderer does not. */
void overnight_vt_resize(void *handle, uint16_t columns, uint16_t rows);
 ...
 */
uint64_t overnight_vt_revision(void *handle);
 ...
 * nothing.
 */
bool overnight_vt_snapshot(void *handle, OvernightVtSnapshot *out);
 ...
 */
void overnight_vt_scroll(void *handle, int32_t lines);
 ...
 */
void overnight_vt_scroll_to_bottom(void *handle);
 ...
 */
size_t overnight_vt_take_writes(void *handle, uint8_t *out, size_t capacity);

/* Take the bell flag, clearing it. */
bool overnight_vt_take_bell(void *handle);

/*
 * The title the program last set, or NULL if it never set one. Borrowed;
 * valid until the next feed.
 */
const char *overnight_vt_title(void *handle);
 ...
 */
bool overnight_vt_alt_screen(void *handle);
 ...
 */
#define OVERNIGHT_VT_KEY_ENTER     0xE000u
#define OVERNIGHT_VT_KEY_TAB       0xE001u
#define OVERNIGHT_VT_KEY_BACKSPACE 0xE002u
#define OVERNIGHT_VT_KEY_ESCAPE    0xE003u
#define OVERNIGHT_VT_KEY_UP        0xE004u
#define OVERNIGHT_VT_KEY_DOWN      0xE005u
#define OVERNIGHT_VT_KEY_RIGHT     0xE006u
#define OVERNIGHT_VT_KEY_LEFT      0xE007u
#define OVERNIGHT_VT_KEY_HOME      0xE008u
#define OVERNIGHT_VT_KEY_END       0xE009u
#define OVERNIGHT_VT_KEY_PAGE_UP   0xE00Au
#define OVERNIGHT_VT_KEY_PAGE_DOWN 0xE00Bu
#define OVERNIGHT_VT_KEY_INSERT    0xE00Cu
#define OVERNIGHT_VT_KEY_DELETE    0xE00Du
/* F1..F12 are OVERNIGHT_VT_KEY_F1 + n - 1. */
#define OVERNIGHT_VT_KEY_F1        0xE010u

#define OVERNIGHT_VT_MOD_SHIFT (1u << 0)
#define OVERNIGHT_VT_MOD_ALT   (1u << 1)
#define OVERNIGHT_VT_MOD_CTRL  (1u << 2)

#define OVERNIGHT_VT_MOUSE_LEFT       0u
#define OVERNIGHT_VT_MOUSE_MIDDLE     1u
#define OVERNIGHT_VT_MOUSE_RIGHT      2u
#define OVERNIGHT_VT_MOUSE_WHEEL_UP   3u
#define OVERNIGHT_VT_MOUSE_WHEEL_DOWN 4u

#define OVERNIGHT_VT_MOUSE_PRESS   0u
#define OVERNIGHT_VT_MOUSE_RELEASE 1u
#define OVERNIGHT_VT_MOUSE_MOVE    2u
 ...
 */
size_t overnight_vt_encode_key(void *handle, uint32_t key, uint32_t modifiers,
 ...
 */
size_t overnight_vt_encode_mouse(void *handle, uint32_t button, uint32_t action,
 ...
 */
size_t overnight_vt_encode_paste(void *handle, const uint8_t *text, size_t len,
                                 uint8_t *out, size_t capacity);
