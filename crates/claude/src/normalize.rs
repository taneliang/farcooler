...
fn user_to_events(frame: &serde_json::Value, origin: Origin) -> Vec<AgentEvent> {
    ...
    if let Some(text) = frame["message"]["content"].as_str() {
        // The CLI's own markup is resolved before the emptiness check, because
        // a record that was NOTHING but a caveat has nothing left to draw and
        // an empty bubble is worse than the tags were.
        let text = farcooler_agent_core::markup::without_harness_markup(text);
        if origin == Origin::Live || text.is_empty() {
            return Vec::new();
        }
        return vec![AgentEvent::Message { role: Role::User, text, parent }];
    }
    let Some(blocks) = frame["message"]["content"].as_array() else { return Vec::new() };
        ...
        .filter_map(|block| {
            if block["type"].as_str() != Some("tool_result") {
                ...
                if origin == Origin::Replay && block["type"].as_str() == Some("text") {
                    let text = farcooler_agent_core::markup::without_harness_markup(
...
mod tests {
    ...
    /// The CLI's own markup never belonged on screen.
    ///
    /// Both shapes a restored prompt arrives in: the on-disk transcript writes
    /// a bare string, the wire writes content blocks.
    #[test]
    fn a_restored_prompt_arrives_without_the_clis_own_markup() {
        ...
        let bare = serde_json::json!({ "type": "user", "message": { "content": caveat } });
        assert_eq!(frame_to_events_from(&bare, Origin::Replay), Vec::new());
        ...
        assert!(matches!(
            frame_to_events_from(&blocks, Origin::Replay).as_slice(),
            [AgentEvent::Message { role: Role::User, text, .. }] if text == "/model haiku"
        ));
    }

    /// The agent's own words are NOT rewritten. It has to be able to quote the
    /// markup — this file's tests are full of it — and its text streams a delta
    /// at a time, where half a tag would be all a rewrite could see.
    #[test]
    fn the_agents_own_words_keep_every_tag_they_contain() {
        let quoted = "Claude Code writes <system-reminder>…</system-reminder> into the record.";
        assert!(matches!(
            frame_to_events_from(&said(quoted), Origin::Replay).as_slice(),
            [AgentEvent::Message { role: Role::Agent, text, .. }] if text == quoted
        ));
    }
