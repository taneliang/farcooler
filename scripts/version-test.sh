...
# is not a cosmetic version string. A beta that reports `release` installs over
# the release daemon and writes its database.
#
...
dir="$(scratch)"
check "untagged is dev" "dev" "$(at "$dir" channel)"
rm -rf "$dir"

# --- a beta tag is beta ---------------------------------------------------
dir="$(scratch)"
git -C "$dir" tag -a v0.2.0-beta.3 -m b
check "a beta tag is beta" "beta" "$(at "$dir" channel)"
rm -rf "$dir"

# --- a release tag is release --------------------------------------------
dir="$(scratch)"
git -C "$dir" tag -a v0.2.0 -m r
check "a release tag is release" "release" "$(at "$dir" channel)"
rm -rf "$dir"

# --- THE PROMOTION CASE ---------------------------------------------------
#
# Promoting a beta puts a release tag on the same commit, because the two have
...
# anyone rebuilding the beta from it.
dir="$(scratch)"
git -C "$dir" tag -a v0.2.0-beta.3 -m b
git -C "$dir" tag -a v0.2.0 -m r
check "a promoted commit built as its beta tag is beta" \
  "beta" "$(at "$dir" channel v0.2.0-beta.3)"
check "a promoted commit built as its release tag is release" \
  "release" "$(at "$dir" channel v0.2.0)"
check "a promoted beta still says which beta it was" \
  "0.2.0 (beta 3)" "$(at "$dir" display v0.2.0-beta.3)"
rm -rf "$dir"
...
check "a dirty tree is dev even at a release tag" "dev" "$(at "$dir" channel)"
rm -rf "$dir"
...
dir="$(scratch)"
check "a name that is not a version tag is dev" "dev" "$(at "$dir" channel nonsense)"
rm -rf "$dir"
...
check "FARCOOLER_TAG decides the channel" \
  "beta" "$(cd "$dir" && FARCOOLER_TAG=v0.2.0-beta.3 ./scripts/version.sh channel)"
check "an explicit argument still wins over the variable" \
  "release" "$(cd "$dir" && FARCOOLER_TAG=v0.2.0-beta.3 ./scripts/version.sh channel v0.2.0)"
rm -rf "$dir"
