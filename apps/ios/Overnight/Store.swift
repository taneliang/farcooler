    ...
    static var publicKey: String? {
        ...
        let written = key.withCString { text in
            buffer.withUnsafeMutableBufferPointer {
                overnight_client_public_key(text, $0.baseAddress, $0.count)
            }
        }
...
extension Host {
    ...
    ///
    /// `-overnightDemoHost user@address:port`, which `UserDefaults` exposes for
    ...
    static func fromLaunchArgument() -> Host? {
        guard let value = UserDefaults.standard.string(forKey: "overnightDemoHost"),
