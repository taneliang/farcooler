...
/// Overnight reaches other machines over ssh. That explanation is still true and
...
/// Overnight was installed, whether the install matched this Mac, and what to
/// type to fix any of it.
///
/// So this asks the host instead. `overnight host probe` changes nothing and
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text(
                    "A user@host or an ssh config alias. Overnight reaches every machine "
                        + "over ssh — there is no Overnight listener to set up anywhere, so a "
