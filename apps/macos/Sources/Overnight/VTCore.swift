...
final class VTCore {
    ...
    var revision: UInt64 {
        ...
        return overnight_vt_revision(handle)
    }

    var isAlternateScreen: Bool {
        guard let handle else { return false }
        return overnight_vt_alt_screen(handle)
    }
    ...
    func scrollToBottomIfScrolled() {
        ...
        guard overnight_vt_snapshot(handle, &raw), raw.display_offset > 0 else { return }
    ...
    func takePendingWrites() -> [UInt8] {
        ...
        while true {
            let n = buffer.withUnsafeMutableBufferPointer {
                overnight_vt_take_writes(handle, $0.baseAddress, $0.count)
            }
    ...
    func takeBell() -> Bool {
        guard let handle else { return false }
        return overnight_vt_take_bell(handle)
    }

    var title: String? {
        guard let handle, let ptr = overnight_vt_title(handle) else { return nil }
    ...
    func encode(paste text: String) -> [UInt8] {
        ...
        let needed = input.withUnsafeMutableBufferPointer {
            overnight_vt_encode_paste(handle, $0.baseAddress, $0.count, nil, 0)
        }
...
extension OvernightVtCell {
    ...
    var isItalic: Bool { flags & UInt16(OVERNIGHT_VT_FLAG_ITALIC) != 0 }
    var isUnderlined: Bool { flags & UInt16(OVERNIGHT_VT_FLAG_UNDERLINE) != 0 }
